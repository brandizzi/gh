...
PullRequest.prototype._commentHandler = function () {
    ...
    var number = options.number ? options.number : options.submittedPullNumber;

    logger.log('Adding comment on pull request ' + logger.colors.green('#' + number));

    this.issue.comment(function (err, pull) {
        if (err) {
            logger.error('Can\'t comment on pull request ' + number + '.');
            return;
        }
